 ...
 * vertice forma um arco que também vao sendo adicionadas conforme 
...
/*-----------------------------------------*/
/*Verficaçao de ciclos*/

int DIGRAPHcycle (Digraph G, int ts[]) {     
    int v;
    time = 0;	
    cnt = G->V-1;
    for(v = 0; v < G->V; v++) 
...
int cycleR (Digraph G, int v, int ts[]) {
    link p; 
    int w;
    d[v] = time++; 
...
void TSdfsR (Digraph G, int v, int ts[]) {
    link p; 
    lbl[v] = 0; 
    for (p = G->adj[v]; p != NULL; p = p->next)
        if (lbl[Key(p->w)] == -1) 
