...
__device__ static const BYTE aes_sbox[16][16] = {
...
static const BYTE aes_sbox_h[16][16] = {
    {0x63,0x7C,0x77,0x7B,0xF2,0x6B,0x6F,0xC5,0x30,0x01,0x67,0x2B,0xFE,0xD7,0xAB,0x76},
    {0xCA,0x82,0xC9,0x7D,0xFA,0x59,0x47,0xF0,0xAD,0xD4,0xA2,0xAF,0x9C,0xA4,0x72,0xC0},
    {0xB7,0xFD,0x93,0x26,0x36,0x3F,0xF7,0xCC,0x34,0xA5,0xE5,0xF1,0x71,0xD8,0x31,0x15},
    {0x04,0xC7,0x23,0xC3,0x18,0x96,0x05,0x9A,0x07,0x12,0x80,0xE2,0xEB,0x27,0xB2,0x75},
...
WORD SubWord(WORD word)
{
    ...
    result = (int)aes_sbox_h[(word >> 4) & 0x0000000F][word & 0x0000000F];
    result += (int)aes_sbox_h[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F] << 8;
    result += (int)aes_sbox_h[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F] << 16;
    result += (int)aes_sbox_h[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F] << 24;
    return(result);
}
...
__global__ void aes_encrypt(const BYTE *in_data, BYTE *out_data, const WORD *key, int keysize, size_t size)
{
  int index = (blockDim.x * blockIdx.x + threadIdx.x) * AES_BLOCK_SIZE;
  if(index < size) {
    BYTE state[4][4];
...
__global__ void aes_decrypt(const BYTE *in_data, BYTE *out_data, const WORD *key, int keysize, size_t size)
{
  int index = (blockDim.x * blockIdx.x + threadIdx.x) * AES_BLOCK_SIZE;
  if(index < size) {
    BYTE state[4][4];
