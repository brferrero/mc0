...
/**************************** VARIABLES *****************************/
__device__ static const WORD p_perm[18] = {
...
__device__ static const WORD s_perm[4][256] = { {
...
__global__ void blowfish_encrypt(const BYTE *in_data, BYTE *out_data, const BLOWFISH_KEY *keystruct, size_t size)
{
   int index = (blockDim.x * blockIdx.x + threadIdx.x) * BLOWFISH_BLOCK_SIZE;
   if(index < size) {
      WORD l,r,t; //,i;

      BYTE in[BLOWFISH_BLOCK_SIZE];
      BYTE out[BLOWFISH_BLOCK_SIZE];
      for(int j = 0; j < BLOWFISH_BLOCK_SIZE; j++) {
         in[j] = in_data[index+j];
      }
      ...
      for(int j = 0; j < BLOWFISH_BLOCK_SIZE; j++) {
         out_data[index+j] = out[j];
      }
   }
}

__global__ void blowfish_decrypt(const BYTE *in_data, BYTE *out_data, const BLOWFISH_KEY *keystruct, size_t size)
{
   int index = (blockDim.x * blockIdx.x + threadIdx.x) * BLOWFISH_BLOCK_SIZE;
   if(index < size) {
      WORD l,r,t; //,i;
...
/*void blowfish_key_setup(const BYTE user_key[], BLOWFISH_KEY *keystruct, size_t len)
{
   ...
   BYTE *d_block;
   cudaMalloc((void **)&d_block, sizeof(block));
   cudaMemcpy(d_block, block, sizeof(block), cudaMemcpyHostToDevice);

   BLOWFISH_KEY *d_keystruct;
   cudaMalloc((void **)&d_keystruct, sizeof(BLOWFISH_KEY));
   cudaMemcpy(d_keystruct, keystruct, sizeof(BLOWFISH_KEY), cudaMemcpyHostToDevice);

   for (idx = 0; idx < 18; idx += 2) {
      blowfish_encrypt<<<1,1>>>(block,block,keystruct,sizeof(block));
      cudaMemcpy(keystruct, d_keystruct, sizeof(BLOWFISH_KEY), cudaMemcpyDeviceToHost);
      keystruct->p[idx] = (block[0] << 24) | (block[1] << 16) | (block[2] << 8) | block[3];
      keystruct->p[idx+1]=(block[4] << 24) | (block[5] << 16) | (block[6] << 8) | block[7];
   }
   // Recalculate the S-boxes.
